...
def extract_nihss(text: str) -> Dict[str, Any]:
    prompt = load_prompt("scales_nihss").replace("{texto}", text)
    return safe_parse_json(_call_llm(prompt))


def extract_mrs(text: str) -> Dict[str, Any]:
    prompt = load_prompt("scales_mrs").replace("{texto}", text)
    return safe_parse_json(_call_llm(prompt))


def extract_scales(text_path: Path) -> Dict[str, Any]:
    """
    Devolve dict aninhado:
    {
    ...
    return {
        "nihss": {k: {"value": v} for k, v in raw_nihss.items()},
        "mrs":   {k: {"value": v} for k, v in raw_mrs.items()},
    }